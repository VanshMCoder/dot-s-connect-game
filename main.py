...
# interface
pygame.display.set_caption("Dot's Connect Game")
...
def show_scores(data):
    text = font2.render('Scoreboard', True, (117, 34, 34), (250, 205, 205))
    text_rect = text.get_rect()
    text_rect.center = (80, 170)
    screen.blit(text, text_rect)
    for player in list(range(data.num_of_players)):
        text = font.render(str(data.scores[player]), True, data.colors[player], (250, 205, 205))
        text_rect = text.get_rect()
        text_rect.center = scores_pos[player]
        screen.blit(text, text_rect)
    text = font.render('TOP', True, data.colors[data.scores.index(max(data.scores))], (250, 205, 205))
    text_rect = text.get_rect()
    text_rect.center = (720, 270)
    screen.blit(text, text_rect)
    text = font1.render('P' + str(data.scores.index(max(data.scores)) + 1), True,
                        data.colors[data.scores.index(max(data.scores))], (250, 205, 205))
    text_rect = text.get_rect()
...
def show_current_player(data):
    text = font.render('P' + str(data.playing + 1), True, data.colors[data.playing], (250, 205, 205))
    text_rect = text.get_rect()
...
def draw_structure():
    screen.fill((250, 205, 205))
    text = font2.render(' Main Menu ', True, (250, 205, 205), (117, 34, 34))
    text_rect = text.get_rect()
...
def draw_result(data):
    ...
    if data.scores.count(max(data.scores)) == 1:
        ...
        text = font1.render('Match drawn', True, (0, 0, 0), (250, 205, 205))
        text_rect = text.get_rect()
        text_rect.center = (400, 160)
        screen.blit(text, text_rect)
    text = font.render(' Main Menu ', True, (0, 0, 0), (171, 69, 69))
...
def draw_menu():
    ...
    image = pygame.image.load("dots.png").convert_alpha()
    img = pygame.transform.scale(img, (600, 400))
    img_rect = img.get_rect()
    img_rect.center = (400, 100)
    screen.blit(img, img_rect)
    text = font.render(' Play Game ', True, (250, 205, 205), (171, 69, 69))
    text_rect = text.get_rect()
...
def draw_select_num_of_players():
    screen.fill((250, 205, 205))
    text = font1.render('Select number of players:', True, (171, 69, 69), (250, 205, 205))
    text_rect = text.get_rect()
    text_rect.center = (400, 280)
    screen.blit(text, text_rect)
    text = font.render(' 2 Players ', True, (250, 205, 205), (171, 69, 69))
    text_rect1 = text.get_rect()
    text_rect1.center = (200, 550)
    screen.blit(text, text_rect1)
    text = font.render(' 3 Players ', True, (250, 205, 205), (171, 69, 69))
    text_rect2 = text.get_rect()
    text_rect2.center = (400, 550)
    screen.blit(text, text_rect2)
    text = font.render(' 4 Players ', True, (250, 205, 205), (171, 69, 69))
